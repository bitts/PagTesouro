...
jQuery(function($){
    ...
    $.get('https://api.github.com/repos/bitts/pagtesouro', function (response) {
        ...
        // https://api.github.com/repos/bitts/PagTesouro/tags (buscar daqui ultima versão)
        ...
        $('#update').append(
            ...
            $('<p />').append(
                'Data da última atualização dos fontes do sistema: ',                
                git_update.getDate()+ '/' + (git_update.getMonth()+1) + '/'+ git_update.getFullYear(),
                ' ('+ git_days_lastupdate + ' dias atrás) *Mantenha seu plugin atualizado'
            ),
            $('<h3 />').append(
    ...
    $.ajax({
        url: 'index.php?option=com_pagtesouro&format=json&get=true&select=true',
        type: 'post',
        success: function(resposta){
            
            let obj = JSON.parse(resposta.data);
            let data = JSON.stringify(obj, undefined, 4);
            $('textarea.pagtesourojson').val(data);
    
            populate_pagtesouro(data);
        },
    ...
    var btn_save = $('<button />').addClass('backup btn btn-small button-save').append(
        ...
        $.ajax({
            url: 'index.php?option=com_pagtesouro&format=json&get=true&update=true',
            type: 'post',
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                //msg('Erro ao salvar conteudo no arquivo pagtesouro.json '+jqXHR + ' - ' + textStatus+ ' - ' +errorThrown);
                //console.log(jqXHR)
            }
        });
